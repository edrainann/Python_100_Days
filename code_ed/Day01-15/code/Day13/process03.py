...
# @Author  : Edrain
"""
使用subprocess模块中的类和函数来创建和启动子进程，然后通过管道来和子进程通信，这些内容我们不在此进行讲解。
...
它是可以被多个进程共享的队列，底层是通过管道和信号量（semaphore）机制来实现的
"""
...
def sub_task(string):
    global counter
    while counter < 10:
        # PingPongPongPingPongPingPongPingPingPongPongPingPongPingPingPongPongPingPingPong
